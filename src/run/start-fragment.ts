...
export function startFragment(
  ...
  const IS_DEVELOPMENT = options.debug;
  ...
  console.log("builtFragment: " + builtFragment);
  ...
  podlet.js({
    value: `http://localhost:${options.port}${builtFragment.replace(
      "out",
      ""
    )}`,
    ...jsSalt,
